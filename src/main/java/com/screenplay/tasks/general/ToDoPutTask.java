...
public class ToDoPutTask implements Task {

    private final RequestStructureModel requestStructureModel;

    public ToDoPutTask(RequestStructureModel requestStructureModel) {
        this.requestStructureModel = requestStructureModel;
    }

    public static ToDoPutTask doPut(RequestStructureModel requestStructureModel) {
        return instrumented(ToDoPutTask.class, requestStructureModel);
    }

    @Override
    public <T extends Actor> void performAs(T actor) {
        RequestSpecification request = actor.asksFor(SetRequestStructureQuestion.withData(requestStructureModel));
        request.given().when().put();
    }
}
